    ...
    for(Synety_Plugin__c sp : Trigger.new)
    {
        ...
        if(sp.QA_Status__c == 'App Approved and Live' && sp.Software_Ready_for_Release__c == 'Yes')
        {
            ...
            if(attachment.size() > 0)
            {
                ...
                doc.folderid = [SELECT AccessType, Id, DeveloperName, Name, NamespacePrefix, Type FROM Folder where name = 'PLUGINS: Download'].id;
                ...
                {
                    trigger.new[0].addError('Deleting Attachment File Failed - ' + attachment[0].name + ' ' + attachment[0].id);
                } 
