trigger OppBeforeUpdate on Opportunity (before update) 
{
    ...
    {
        /*
        if(Trigger.oldMap.get(opp.id).CloseDate != opp.CloseDate && opp.Order_Processed__c)
        */
        ...
        if(Trigger.oldMap.get(opp.id).CloseDate != opp.CloseDate && opp.Next_Steps__c == 'Customer - Provisioned')
        {
            opp.CloseDate.addError('You cannot change the close date of an order once it has been provisioned.');
        }
        
        if(Trigger.oldMap.get(opp.id).Next_Steps__c != opp.Next_Steps__c && opp.Next_Steps__c == 'Customer - Provisioned')
        {
