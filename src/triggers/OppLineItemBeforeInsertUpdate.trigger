trigger OppLineItemBeforeInsertUpdate on OpportunityLineItem (before insert, before update) 
{
    ...
    for(OpportunityLineItem oli: trigger.New)
    {
        if(oppIdsAlreadyProcessed.contains(oli.OpportunityId) && ((UserInfo.getUserName().contains('mohsin.raza@synety.com') == false) && (UserInfo.getUserName().contains('rahul.borgaonkar@synety.com') == false)))
        {
